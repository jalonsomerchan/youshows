...
Cada serie contiene descripción, arte, géneros, tags, idioma, edad recomendada y una lista de temporadas. `language` utiliza códigos estables como `es`, `es-419` (Español Latino), `ca`, `en` o `none`; `ageRating` admite `all`, `3`, `7`, `12`, `16`, `18` o `none`. Las etiquetas visibles se traducen en la interfaz.
